 ...
 */
import { CasConflictError } from "./state/store.js";
const ALLOWED = {
    INACTIVE: ["DOCTOR"],
...
export class PhaseMachine {
    ...
    current() {
        ...
        if (!s)
            throw new Error("No workflow state; call tdd_start first.");
        return s;
    }
    /**
    ...
    advance(input) {
        ...
        if (state.activeLease)
            throw new LeaseHeldError(state.activeLease);
    ...
    releaseLease(callID) {
        const state = this.current();
        if (!state.activeLease || state.activeLease.callID !== callID)
            return state;
        const draft = structuredClone(state);
