 ...
 */
function candidateHomes(env) {
    const homes = [];
    const push = (p) => {
        if (p && !homes.includes(p))
            homes.push(p);
    };
    push(env.JAVA_HOME);
    push(env.JDK_HOME);
...
export function discoverToolchain(env = process.env) {
    ...
    return {
        ok: false,
        reason: "No JDK with `javac` found. A JRE-only JAVA_HOME cannot compile and will fail before tests run (classified ENV_FAILURE). Install a JDK (the project requires one) or point JAVA_HOME at one.",
        candidatesTried: tried,
    };
}
