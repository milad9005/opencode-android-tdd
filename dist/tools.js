...
import { runTargetedTest } from "./gradle/runner.js";
...
function requireState(store) {
    const s = store.read();
    if (!s)
        throw new Error("No active TDD workflow. Call tdd_start first.");
    return s;
}
...
export function createTools(deps) {
    ...
    const tdd_plan_set = tool({
        ...
        async execute(args) {
            for (const sl of args.slices) {
                const bad = [...sl.allowedProductionPaths, ...sl.allowedTestFiles].find((p) => p.includes("*") || p.trim() === "" || p.endsWith("/") || p === "src" || p === "src/main");
                if (bad)
                    throw new Error(`Slice '${sl.id}' has an over-broad/wildcard path '${bad}'. Name concrete files only.`);
            }
            return withLock(store, () => {
                const s = requireState(store);
                ...
                const draft = structuredClone(s);
                draft.slices = args.slices.map((sl) => ({ ...sl, status: "pending" }));
    ...
    function runForActiveSlice(s) {
        const slice = activeSlice(s);
        if (!slice)
            throw new Error("No active slice.");
        const javaHome = deps.toolchainJavaHome();
        if (!javaHome)
            throw new Error("No JDK toolchain resolved. Run tdd_doctor.");
    ...
    const tdd_baseline = tool({
        ...
        async execute() {
            const s = requireState(store);
            if (s.phase !== "BASELINE")
                throw new Error(`tdd_baseline requires phase BASELINE, not ${s.phase}.`);
    ...
    const tdd_verify_red = tool({
        ...
        async execute() {
            const s = requireState(store);
            if (s.phase !== "TEST_WRITE")
                throw new Error(`tdd_verify_red requires phase TEST_WRITE, not ${s.phase}.`);
            const slice = activeSlice(s);
            if (!slice)
                throw new Error("No active slice.");
            const outcome = await runForActiveSlice(s);
    ...
    const tdd_verify_green = tool({
        ...
        async execute() {
            const s = requireState(store);
            if (s.phase !== "IMPL" && s.phase !== "REFACTOR")
                throw new Error(`tdd_verify_green requires phase IMPL/REFACTOR, not ${s.phase}.`);
            const slice = activeSlice(s);
            if (!slice)
                throw new Error("No active slice.");
            if (s.redProof) {
    ...
    const tdd_inspect_done = tool({
        ...
        async execute() {
            return withLock(store, () => {
                const s = requireState(store);
                if (s.phase !== "INSPECT")
                    throw new Error(`tdd_inspect_done requires phase INSPECT, not ${s.phase}.`);
                const draft = structuredClone(s);
    ...
    const tdd_takeover_stale_lock = tool({
        ...
        async execute() {
            ...
            try {
                const s = store.read();
                if (s)
                    ledger.append({ workflowId: s.workflowId, stateVersion: s.stateVersion, type: "LOCK_TAKEOVER", phase: s.phase, detail: { previousLockAgeMs: age } });
    ...
    const tdd_explain_block = tool({
        ...
        async execute() {
            const s = store.read();
            if (!s)
                return "No workflow. Writes are denied until tdd_start.";
            const slice = activeSlice(s);
            const lines = [`phase=${s.phase}, slice=${slice?.id ?? "-"}`];
            switch (s.phase) {
                case "TEST_WRITE":
                    lines.push("Editable: slice test files only. Next: tdd_run then tdd_verify_red.");
                    break;
                case "IMPL":
                    lines.push(s.redProof ? "Editable: slice production paths only (no test edits). Next: tdd_verify_green." : "Blocked: no redProof. Next: tdd_verify_red.");
                    break;
                case "REFACTOR":
                    lines.push("Editable: slice test+prod (green-gated). Next: tdd_verify_green or tdd_inspect_done.");
                    break;
                case "BASELINE":
                    lines.push("No edits. Next: tdd_baseline.");
                    break;
    ...
    const tdd_report = tool({
        ...
        async execute() {
            ...
            return withLock(store, () => {
                const cur = requireState(store);
                const draft = structuredClone(cur);
                if (cur.phase === "REGRESSION_GATE" || cur.phase === "ARCH_GATE" || cur.phase === "REPORT")
                    draft.phase = "DONE";
    ...
    const tdd_allow_build_edit = tool({
        ...
        async execute(args) {
            ...
            return withLock(store, () => {
                ...
                draft.redProof = undefined;
                draft.greenVerifiedAt = undefined;
                if (hadProof && (draft.phase === "IMPL" || draft.phase === "REFACTOR"))
                    draft.phase = "TEST_WRITE";
                const committed = store.commit(cur.stateVersion, draft);
    ...
    const tdd_expand_scope = tool({
        ...
        async execute(args) {
            ...
            return withLock(store, () => {
                ...
                const slice = activeSlice(draft);
                if (!slice)
                    throw new Error("No active slice to expand.");
                ...
                draft.redProof = undefined;
                draft.greenVerifiedAt = undefined;
                if (draft.phase === "IMPL" || draft.phase === "REFACTOR")
                    draft.phase = "TEST_WRITE";
                const committed = store.commit(s.stateVersion, draft);
