 ...
 */
import { classifyTool, isKnownDenied } from "./buckets.js";
import { classifyPath, isBucketEscape } from "./paths.js";
import { detectDrift } from "./hash.js";
function deny(bucket, reason, message, pathBucket) {
    return { decision: "DENY", bucket, pathBucket, reason, message };
}
function allow(bucket, reason, pathBucket) {
...
function pathInList(filePath, worktree, list) {
...
const MUTATING_PHASES = ["TEST_WRITE", "IMPL", "REFACTOR", "REPORT"];
export function decideGate(input) {
    const bucket = classifyTool(input.tool);
    // 1) Read-only is always allowed (incl. all subagent work).
    if (bucket === "read-only")
        return allow(bucket, "read-only tool");
    // 2) Subagents are read-only ALWAYS, regardless of phase (Blocker #5).
    if (input.isSubagent) {
        return deny(bucket, "subagent-mutation", `TDD gate: subagents are read-only. The '${input.tool}' tool cannot mutate state from a delegated subagent. Run mutations from the primary android-tdd agent.`);
    }
    // 3) Plugin-owned tools are the only path to Gradle + transitions.
    if (bucket === "plugin-owned")
        return allow(bucket, "plugin-owned tdd_* tool");
    ...
    if (phase === "REFACTOR") {
        ...
        const inScope = (pb === "test" && pathInList(input.filePath, input.worktree, slice.allowedTestFiles)) ||
