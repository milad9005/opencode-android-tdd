 ...
 */
export function buildConfigHash(worktree, buildFiles) {
...
export function detectDrift(worktree, recorded) {
    const changed = [];
    for (const [p, recordedHash] of Object.entries(recorded)) {
        if (hashFile(abs(worktree, p)) !== recordedHash)
            changed.push(p);
    }
