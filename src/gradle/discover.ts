...
export interface ModuleModel {
  path: string; // gradle path e.g. ":common:regex"
  kind: ModuleKind;
...
function extractTaskNames(stdout: string): string[] {
 ...
 */
function isPlainBuildTypeVariant(variant: string, knownBuildTypes: string[]): boolean {
...
export function discoverModule(input: DiscoverInput): ModuleModel {
  ...
  const kmpHit =
    (input.sourceSetsStdout !== undefined && RE_KMP_TASK.test(input.sourceSetsStdout)) ||
    RE_KMP_TASK.test(input.tasksStdout);

  const unsupported = (u: UnsupportedReason): ModuleModel => ({
    path: input.path,
