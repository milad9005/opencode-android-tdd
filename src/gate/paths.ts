...
export interface PathClassifyInput {
   ...
   */
  sourceSetRoots?: { testRoots: string[]; mainRoots: string[] };
}
...
function toAbs(filePath: string, worktree: string): string {
...
function underAnyRoot(realPath: string, roots: string[]): boolean {
...
export function classifyPath(input: PathClassifyInput): PathBucket {
  ...
  if (input.sourceSetRoots) {
    if (underAnyRoot(real, input.sourceSetRoots.testRoots)) return "test";
    if (underAnyRoot(real, input.sourceSetRoots.mainRoots)) return "main";
  }

  // Build-logic (stronger gate than ordinary build files): buildSrc, build-logic,
  // convention plugins, included builds.
  if (BUILD_LOGIC_SEGMENTS.some((s) => hasSegment(normalized, s))) return "build-logic";

  // Build config files.
  if (BUILD_FILE_NAMES.includes(base)) return "build";
  ...
  if (isTestPath) return "test";

  // Production source.
  if (hasSegment(normalized, "main") || hasSegment(normalized, "src")) return "main";
 ...
 */
export function isBucketEscape(input: PathClassifyInput): boolean {
  const abs = toAbs(input.filePath, input.worktree);
  const real = safeRealpath(abs);
  ...
  if (looksTest && classifyPath(input) === "main") return true;
  return false;
}
