...
const GUARDED_MUTATORS = new Set<string>(["write", "edit"]);
 ...
 */
const KNOWN_DENIED = new Set<string>([
  "bash",
  "patch",
  "apply",
  "move",
...
export function classifyTool(tool: string): ToolBucket {
  if (tool.startsWith(PLUGIN_OWNED_PREFIX)) return "plugin-owned";
  if (READ_ONLY.has(tool) || READ_ONLY_PREFIXES.some((p) => tool.startsWith(p))) {
    return "read-only";
  }
  if (GUARDED_MUTATORS.has(tool)) return "guarded-mutator";
  // KNOWN_DENIED falls through here too; everything unrecognized => denied.
  return "denied";
}

export function isKnownDenied(tool: string): boolean {
